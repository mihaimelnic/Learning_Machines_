...
# here we have tresholds defined. 
collision_threshold = 90
green_threshold = 10
red_threshold = 10

# here we have IR discretisation in simple raw units.
ir_near = 200
ir_mid   = 80
...
def do_move(rob, action):
    ...
    duration_ms = 500  # forward about 0.5 s.
    ...
    if is_hardware:
        # We sse the public move API if available; fall back to service call.
        try:
...
def calculate_reward(state_tuple, center_ir, action, food_delta=0):
    ...
    elif colour == "RG":             # BOTH red and green visible! HERE
